...
class BodySkeleton: Entity {
    ...
    required init(for bodyAnchor: ARAnchor) {
        super.init()
        
//        guard let tsample = try? Entity.load(named: "tsample.scnassets/tsample") else {
//            return
//        }
//        
//        
//        self.addChild(tsample)
        ...
        for bone in Bones.allCases {
            guard let skeletonBone = createSkeletonBone(bone: bone, bodyAnchor: bodyAnchor as! ARBodyAnchor)
            else {continue}
    ...
    private func createSkeletonBone(bone: Bones, bodyAnchor: ARBodyAnchor) -> SkeletonBone? {
        guard let fromJointEntityTransform = bodyAnchor.skeleton.modelTransform(for:
                                                                                    ARSkeleton.JointName (rawValue: bone.jointFromName)),
              let toJointEntityTransform = bodyAnchor.skeleton.modelTransform(for:
                                                                                ARSkeleton.JointName (rawValue: bone.jointToName))
        else { return nil }
